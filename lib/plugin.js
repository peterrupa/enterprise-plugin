...
class ServerlessEnterprisePlugin {
  constructor(sls) {
    ...
    // set org & app from CLI flags -- except for interactive cli, it checks options explicitly
    if (sls.processedInput.options.org && !interactiveCliHooks) {
      // in anticipation of eventually using org internally too..
      sls.service.org = sls.processedInput.options.org;
      sls.service.tenant = sls.processedInput.options.org;
    }
    if (sls.processedInput.options.app && !interactiveCliHooks) {
